...
=item B<-Wl,-(no-|disable-)[dynamicbase | nxcompat | high-entropy-va | tsaware]>

Set or clear PE (Windows) executable header hardening flags.  The
B<-Wl,-high-entropy-va> option is supported on x86-64 and ARM64 PE
targets and implies B<-Wl,-dynamicbase>.  Clearing dynamicbase also
clears high-entropy-va.  When B<-Wl,-dynamicbase> is used for an
