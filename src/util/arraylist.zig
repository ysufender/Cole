...
const PointList = MultiArrayList(Point);

test "append + get roundtrip" {
    var list = try PointList.init(testing.allocator, 4);
    defer {
        var s = list.slice();
        s.free(testing.allocator);
    }

    try list.append(testing.allocator, .{ .x = 1, .y = -1 });
    try list.append(testing.allocator, .{ .x = 2, .y = -2 });

    try testing.expectEqual(@as(i32, 1), list.get(0).x);
    try testing.expectEqual(@as(i32, -2), list.get(1).y);
}

test "grows past initial capacity" {
    var list = try PointList.init(testing.allocator, 1);
    defer {
        var s = list.slice();
        s.free(testing.allocator);
    }
    ...
    var i: i32 = 0;
    while (i < 200) : (i += 1) {
        try list.append(testing.allocator, .{ .x = i, .y = -i });
    }

    i = 0;
    while (i < 200) : (i += 1) {
        try testing.expectEqual(i, list.get(@intCast(i)).x);
    }
}

test "set overwrites in place, doesn't shift" {
    var list = try PointList.init(testing.allocator, 4);
    defer {
        var s = list.slice();
        s.free(testing.allocator);
    ...
    try list.append(testing.allocator, .{ .x = 1, .y = 1 });
    try list.append(testing.allocator, .{ .x = 2, .y = 2 });
    list.set(0, .{ .x = 99, .y = 99 });

    try testing.expectEqual(@as(i32, 99), list.get(0).x);
    try testing.expectEqual(@as(i32, 2), list.get(1).x);
}

test "fuzz: append matches a plain shadow array" {
    const S = struct {
        fn run(_: void, input: []const u8) !void {
            var list = try PointList.init(testing.allocator, 2);
            defer {
                var s = list.slice();
                s.free(testing.allocator);
            }
            var shadow = std.ArrayList(Point).empty;
            defer shadow.deinit(testing.allocator);

            for (input) |b| {
                const p = Point{ .x = @intCast(b), .y = -@as(i32, @intCast(b)) };
                try list.append(testing.allocator, p);
                try shadow.append(testing.allocator, p);
            }

            try testing.expectEqual(@as(u32, @intCast(shadow.items.len)), list.len);
            for (shadow.items, 0..) |expected, idx| {
                const got = list.get(@intCast(idx));
                try testing.expectEqual(expected.x, got.x);
                try testing.expectEqual(expected.y, got.y);
            }
        }
    };
    try testing.fuzz({}, S.run, .{});
}
