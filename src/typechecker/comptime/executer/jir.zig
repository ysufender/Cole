...
pub fn executeCall(self: *JIRExecuter, func: *JIR.Function, args: []const Comptime.Value.Ptr) Error!Comptime.Value {
    ...
    variables.ensureTotalCapacity(self.allocator, @intCast(args.len))
...
fn executeBlock(self: *JIRExecuter, blockPtr: JIR.Ptr) Error!Comptime.Value {
...
fn expression(self: *JIRExecuter, exprPtr: JIR.Ptr) Error!Comptime.Value {
...
fn call(self: *JIRExecuter, dataPtr: defines.OpaquePtr) Error!Comptime.Value {
    ...
    const args = self.allocator.alloc(JIR.Ptr, argsLen)
        catch return Error.AllocatorFailure;
...
fn literal(self: *JIRExecuter, constPtr: JIR.Ptr) Error!Comptime.Value {
...
fn decodePushScope(self: *JIRExecuter, blockPtr: JIR.Ptr) Error!void {
    ...
    var variables = Scope.VariableMap.empty;
    variables.ensureTotalCapacity(self.allocator, variableCount)
        catch return Error.AllocatorFailure;

    var labels = Scope.LabelMap.empty;
    labels.ensureTotalCapacity(self.allocator, labelCount)
        catch return Error.AllocatorFailure;
...
fn pushStack(self: *JIRExecuter, scope: Scope) Error!void {
...
fn popStack(self: *JIRExecuter) Error!Scope {
...
pub fn getVar(self: *JIRExecuter, name: defines.StringPtr) Error!Comptime.Value {
...
fn setVar(self: *JIRExecuter, name: defines.StringPtr, newValue: Comptime.Value.Ptr) Error!void {
    var stack = self.stack;
    while (stack.pop()) |scope| {
...
fn getLabel(self: *const JIRExecuter, name: defines.StringPtr) Error!Scope.Symbol {
    var stack = self.stack;
    while (stack.pop()) |scope| {
...
fn report(self: *JIRExecuter, comptime fmt: []const u8, args: anytype) void {
    return self.typechecker.report("COMPTIME EXECUTER (JIR): " ++ fmt, args);
}
